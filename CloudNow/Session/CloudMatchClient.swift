...
private func buildSessionRequestBody(_ input: SessionCreateRequest, deviceId: String) -> [String: Any] {
    ...
    let trueHdr = false
    // Send the raw bit depth / chroma format (sdr8bit → 8/1) to match what buildNvstSdp
    ...
    let cloudMatchBitDepth = input.settings.colorQuality.bitDepth
    let cloudMatchChromaFormat = input.settings.colorQuality.chromaFormat
